...
test("接受至少 32 字元且只含安全字元的家庭 key", () => {
  assert.equal(isValidFamilyKey("Abcd_1234-efgh5678-Ijkl9012-mnop"), true);
...
test("拒絕缺少、過短、過長或含不安全字元的家庭 key", () => {
  ...
  assert.equal(isValidFamilyKey("A".repeat(31)), false);
...
test("從網址解析合法 familyId", () => {
  assert.equal(
    resolveFamilyId("?key=Abcd_1234-efgh5678-Ijkl9012-mnop"),
    "Abcd_1234-efgh5678-Ijkl9012-mnop"
  );
...
test("雲端同步層可設定目前 familyId", () => {
  setFamilyId("Family_1234567890_Abcdefghijklmn");
  assert.equal(getFamilyId(), "Family_1234567890_Abcdefghijklmn");
