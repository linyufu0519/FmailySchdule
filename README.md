...
1. 開啟 iPhone「捷徑」App，新增捷徑，名稱必須是 **新增家庭行程**。
2. 將捷徑設定為接收「文字」輸入。
3. 加入「從輸入取得字典」（Get Dictionary from Input）。
4. 加入「加入新行程」（Add New Event）。
5. 標題使用字典 `title`、開始日期使用 `startAt`、結束日期使用 `endAt`、備註使用 `notes`。
6. 關閉「執行時顯示」（Show When Run）並儲存。
7. 回到網站點「加入手機行事曆」。第一次執行若詢問允許開啟捷徑或存取行事曆，請允許。
